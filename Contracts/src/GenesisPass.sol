 ...
 * - Multi-phase sales: Phase 1 (Allowlist Freemint), Phase 2 (Public Paid Mint)
 * - Fixed price minting: 0.005 ETH (~$10) for public phase
...
contract GenesisPass is ERC721, ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    // Minting phases: 0 = Not started, 1 = Allowlist Phase (Freemint), 2 = Public Phase (Paid)
    enum MintPhase {
        NotStarted,
        AllowlistPhase,
        PublicPhase
    ...
    MintPhase public currentPhase = MintPhase.NotStarted;
    ...
    mapping(address => bool) public allowlist; // Allowlist for Phase 1 freemint
    ...
    event MintPhaseChanged(MintPhase oldPhase, MintPhase newPhase);
     ...
     * Supports multi-phase sales:
     * - Phase 1 (AllowlistPhase): Free mint for allowlisted addresses
     * - Phase 2 (PublicPhase): Paid mint for everyone
     * @param to Address to mint the NFT to
     */
    function mint(address to) external payable nonReentrant {
        require(currentPhase != MintPhase.NotStarted, "Minting not started");
        ...
        if (currentPhase == MintPhase.AllowlistPhase) {
            require(allowlist[to], "Not on allowlist");
            require(msg.value == 0, "Allowlist phase is free");
            actualPrice = 0;
        }
        // Phase 2: Public paid mint
        else if (currentPhase == MintPhase.PublicPhase) {
            require(msg.value >= mintPrice, "Insufficient payment");
            actualPrice = mintPrice;
        }
        ...
        // Refund excess payment (only in public phase)
        if (currentPhase == MintPhase.PublicPhase && msg.value > mintPrice) {
            payable(to).transfer(msg.value - mintPrice);
        }

        emit Minted(to, tokenId, actualPrice);
    }
    ...
    /**
     * @dev Set the current minting phase
     * @param phase The new minting phase (0 = NotStarted, 1 = AllowlistPhase, 2 = PublicPhase)
     */
    function setMintPhase(MintPhase phase) external onlyOwner {
        MintPhase oldPhase = currentPhase;
        currentPhase = phase;
        emit MintPhaseChanged(oldPhase, phase);
    }

    /**
     * @dev Add an address to the allowlist
     * @param account The address to add to allowlist
     */
     ...
     * @param account The address to remove from allowlist
     */
    ...
    /**
     * @dev Batch add addresses to allowlist (gas efficient)
    ...
    function batchAddToAllowlist(address[] calldata accounts) external onlyOwner {
        ...
        emit AllowlistBatchUpdated(accounts, true);
    }

    /**
     * @dev Batch remove addresses from allowlist (gas efficient)
    ...
    function batchRemoveFromAllowlist(address[] calldata accounts) external onlyOwner {
        ...
        emit AllowlistBatchUpdated(accounts, false);
    }

    /**
     * @dev Check if an address is on the allowlist
     * @param account The address to check
     * @return isAllowed Whether the address is on allowlist
     */
     ...
     * @param baseURI The base URI (e.g., "https://ipfs.io/ipfs/Qm.../" or "https://api.blockai.com/metadata/")
     */
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        // Return a placeholder if baseURI not set
        // This allows the NFT to be visible in wallets even without metadata
        return string(abi.encodePacked("https://blockai.com/genesis-pass/", Strings.toString(tokenId)));
    }

    /**
     * @dev Withdraw contract funds
     * Uses ReentrancyGuard for additional security
     */
